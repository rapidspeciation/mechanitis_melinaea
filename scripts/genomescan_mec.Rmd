...
filt_scan_long.df %>%
  ...
  theme(text = element_text(size = text_size),
        ...
        strip.background = element_blank(),
        strip.text = element_text(size=text_size, face = "italic"),
        plot.margin = unit(c(10,5,0,13), "pt")
        )
...
ggsave(ggarrange(fst_scan, labels = c("B"), 
         ...
         filename=paste("../../genomescan/output/", genus, "/", comp_taxa, "_fst_scan", Sys.Date(), "_ms.png", sep = ""),
  height = 2.5,
  width = 18
  )
...
ggsave(ggarrange(box_fst, box_dxy, box_pi,
            ncol = 1, align = "v",
            heights = c(1,1.1,1),
            labels = c("C", "", ""),
            font.label = list(size=24,
                              face="plain", family="Calibri")),
  filename=paste("../../genomescan/output/", genus, "/", comp_taxa, "_fst_dxy_pi_", Sys.Date(), "box_ms.png", sep = ""),
  height = 15,
  width = 5.8
  )
